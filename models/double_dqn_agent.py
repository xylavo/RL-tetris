...
class DQNAgent(nn.Module):
    ...
    def train(self):
        ...
        with torch.no_grad():
            next_q = self.forward(next_states_board_tensor, next_states_next_block_tensor, next_states_time_tensor)
        next_target_q = self.forward_target_network(next_states_board_tensor, next_states_next_block_tensor, next_states_time_tensor)

        chosen_q_values = q_values.gather(dim=-1, index=actions_tensor.reshape(-1,1)).reshape(-1)
        next_argmax_actions = next_q.argmax(dim=-1).reshape(-1,1)
        next_target_max_q = next_target_q.gather(dim=-1, index=next_argmax_actions).reshape(-1)
        target_q_values = rewards_tensor + self.config.gamma * (1 - dones_tensor) * next_target_max_q
...
if __name__ == "__main__":
    ...
    for step_train in range(Configuration.train_env_steps):
        ...
        if step_train % Configuration.eval_period == 0:
            ...
            torch.save(agent.target_board_cnn.state_dict(), f"trained/double_dqn_agent_latest_board_{step_train}.pth")
            torch.save(agent.target_next_block_mlp.state_dict(), f"trained/double_dqn_agent_latest_next_block_{step_train}.pth")
            torch.save(agent.target_fc.state_dict(), f"trained/double_dqn_agent_latest_fc_{step_train}.pth")
